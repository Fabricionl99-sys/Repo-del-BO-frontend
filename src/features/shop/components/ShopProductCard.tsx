  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        ...
        <h4 className="line-clamp-2 text-[15px] font-medium">{product.name}</h4>
        <p className="mb-1 font-mono text-[12px] text-text-tertiary">{product.code}</p>
        <p className="mb-2 line-clamp-2 text-[13px] text-text-tertiary">{product.description}</p>
        <div className="flex items-center justify-between">
          <span className="text-mono text-[14px] font-semibold text-accent">
            {formatNumber(product.cost_in_coins)}{' '}
            <span className="text-[13px] text-text-tertiary">{product.currency_code}</span>
          </span>
