      ...
      {item && (
        <div className="space-y-4 text-[13px]">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-[11px] text-text-tertiary">jugador</p>
              <p className="font-medium">{item.player_handle ?? item.player_id}</p>
            </div>
            <div>
              <p className="text-[11px] text-text-tertiary">tipo</p>
              <p className="font-medium">{item.chest_type_name}</p>
              <p className="font-mono text-[10px] text-text-tertiary">{item.chest_type_code}</p>
            </div>
            <div>
              <p className="text-[11px] text-text-tertiary">adquirido</p>
              <p>{formatRelativeDate(item.acquired_at)}</p>
            </div>
            <div>
              <p className="text-[11px] text-text-tertiary">trigger</p>
            ...
            <div>
              <p className="text-[11px] text-text-tertiary">expira</p>
          ...
          {item.prize_snapshot && (
            <div className="rounded-lg border border-border-subtle bg-bg-tertiary p-4">
              ...
              <p className="text-[11px] text-text-secondary">{item.prize_snapshot.reward_type}</p>
              <p className="mt-1 text-[12px] text-text-secondary">
                {summarizeRewardConfig(item.prize_snapshot)}
              </p>
              <pre className="mt-3 overflow-x-auto rounded bg-bg-primary p-2 text-[10px] text-text-tertiary">
