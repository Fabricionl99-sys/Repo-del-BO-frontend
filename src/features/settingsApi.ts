...
export const useOperatorConfig=()=>useQuery({queryKey:['operator-config'],queryFn:()=>apiClient.get('/admin/operator-config').then(r=>readStoredConfig()??unwrapData<OperatorConfig>(r.data))});
export const useSaveOperatorConfig=()=>{const qc=useQueryClient();return useMutation({mutationFn:(payload:Partial<OperatorConfig>)=>apiClient.patch('/admin/operator-config',payload).then(r=>unwrapData<OperatorConfig>(r.data)),onSuccess:(data)=>{storeConfig(data);qc.setQueryData(['operator-config'],data);qc.invalidateQueries({queryKey:['operator-config']})}})};
