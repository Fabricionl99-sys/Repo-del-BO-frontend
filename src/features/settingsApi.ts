...
export const useSaveOperatorConfig=()=>{const qc=useQueryClient();return useMutation({mutationFn:(payload:Partial<OperatorConfig>)=>apiClient.patch('/admin/operator-config',payload).then(r=>r.data as OperatorConfig),onSuccess:(data)=>{qc.setQueryData(['operator-config'],data);qc.invalidateQueries({queryKey:['operator-config']})}})};
