    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-3">
        {(['shop', 'level_up', 'mission', 'chest', 'manual', 'auto'] as const).map((method) => (
          <label key={method} className="flex items-center gap-2 text-[12px]">
      ...
      {unlockMethod === 'shop' && (
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-[12px] text-text-secondary">costo en monedas</label>
            <input type="number" className="field" {...register('shop_cost_in_coins', { valueAsNumber: true })} />
            {errors.shop_cost_in_coins && (
              <p className="mt-1 text-[11px] text-danger">{errors.shop_cost_in_coins.message}</p>
            )}
          </div>
          <div>
      ...
      {unlockMethod === 'level_up' && (
        <div>
          <label className="mb-1 block text-[12px] text-text-secondary">nivel requerido</label>
          <input type="number" className="field" {...register('level_required_level', { valueAsNumber: true })} />
          {errors.level_required_level && (
            <p className="mt-1 text-[11px] text-danger">{errors.level_required_level.message}</p>
          )}
        </div>
      )}
      ...
      {unlockMethod === 'mission' && (
        <div>
          <label className="mb-1 block text-[12px] text-text-secondary">misión</label>
          ...
          {errors.mission_code && <p className="mt-1 text-[11px] text-danger">{errors.mission_code.message}</p>}
        </div>
      )}

      {unlockMethod === 'chest' && (
        <div>
          <label className="mb-1 block text-[12px] text-text-secondary">tipos de cofre</label>
          ...
          {errors.chest_type_codes && (
            <p className="mt-1 text-[11px] text-danger">{errors.chest_type_codes.message}</p>
          )}
        </div>
      )}
      ...
      {unlockMethod === 'auto' && (
        <div>
          <label className="mb-1 block text-[12px] text-text-secondary">disponible desde</label>
          <input type="date" className="field" {...register('auto_from_date')} />
          {errors.auto_from_date && <p className="mt-1 text-[11px] text-danger">{errors.auto_from_date.message}</p>}
        </div>
      )}
    </div>
  );
