...
export type PredictionMarket='result_1x2'|'winner_2options'|'total_goals'|'total_corners'|'both_score'|'exact_score'; export type PredictionEventStatus='active'|'closed_pending_result'|'past'|'draft'; export type Sport='football'|'tennis'|'basketball'|'ufc'|'other';
export interface PredictionMarketDefinition{id:PredictionMarket;label:string;buttons:{value:string;label:string}[]}
export interface PredictionItem{id:string;position:number;name:string;market:PredictionMarket;prize_amount:number;predictions_count:number;predictions_breakdown:{value:string;count:number;percent:number}[];result?:string}
export interface PredictionEvent{id:string;name:string;description:string;sport:Sport;banner_url?:string;status:PredictionEventStatus;closes_at:string;entry_cost:number;items:PredictionItem[];grand_prize_amount:number;grand_prize_chest_id?:string;participants_count:number;pool_accumulated:number;completion_rate:number}
